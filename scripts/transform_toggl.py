...
def derive_time_parts(start_iso: str) -> Tuple[str, int, int, int, int]:
    ...
    try:
        ...
        return (start_date, dt.year, dt.month, dt.day, dt.isocalendar()[1])
    except (ValueError, AttributeError):
...
def _normalize_timestamp_for_key(value: Any) -> str:
    """Return a stable UTC timestamp string for canonical dedupe keys."""
    if not value:
        return ""
...
def build_canonical_entry_key(
    ...
    """Return a stable key for matching CSV and enriched versions of one entry."""
    seed = "|".join(
        [
            _normalize_timestamp_for_key(start_iso),
            _normalize_timestamp_for_key(stop_iso),
            str(duration_sec),
...
def transform_csv_entry(row: dict[str, str]) -> dict[str, Any]:
    ...
    synthetic_id = build_synthetic_id(
        start_iso, stop_iso, description, project_name, duration_sec
    )
    ...
    return {
        ...
        "canonical_key": build_canonical_entry_key(
            start_iso, stop_iso, description, project_name, duration_sec
        ),
        "toggl_id": None,
        "task_id": None,
