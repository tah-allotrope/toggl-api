# DEPRECATED: Legacy Streamlit chat query engine. See functions/chat_engine.py.
"""
...
def _dispatch_question(q: str, conn) -> str:
    ...
    if compare_match:
        return _answer_compare(
            conn, int(compare_match.group(1)), int(compare_match.group(2))
        )

    # ----- "total" / "overall" / "all time" -- but only if not scoped -----
    if any(
        kw in q for kw in ["total", "overall", "how much time", "all time", "lifetime"]
    ):
...
def _answer_date_across_years(conn, month: int, day: int) -> str:
    ...
    for year_val in sorted(df["start_year"].unique()):
        ...
        top_desc = (
            year_df.sort_values("duration_hours", ascending=False).iloc[0][
                "description"
            ]
            or ""
        )
        lines.append(
            f"- **{int(year_val)}:** {total_h:.1f}h -- top project: {top_proj or '(none)'}"
...
def _answer_month(conn, month: int, year: int | None) -> str:
    ...
    proj_lines = "\n".join(
        f"  - {n or '(No Project)'}: {h:.1f}h" for n, h in top.items()
    )
...
def _answer_project(conn, project_name: str, year: int | None) -> str:
    ...
    desc_lines = (
        "\n".join(f"  - {d}: {h:.1f}h" for d, h in top_desc.items())
...
def _answer_top_projects(conn, year: int | None) -> str:
    ...
    for _, row in top.iterrows():
        ...
        lines.append(
            f"  - **{name}:** {row['hours']:,.1f}h ({pct:.1f}%) -- {row['entries']} entries"
        )
...
def _answer_top_tags(conn, year: int | None) -> str:
    ...
    for _, row in top.iterrows():
        lines.append(
            f"  - **{row['tags_list']}:** {row['hours']:,.1f}h -- {row['entries']} entries"
        )
...
def _answer_top_tasks(conn, year: int | None) -> str:
    ...
    for _, row in top.iterrows():
        lines.append(
            f"  - **{row['task_name']}:** {row['hours']:,.1f}h -- {row['entries']} entries"
        )
